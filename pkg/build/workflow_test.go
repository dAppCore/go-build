...
func TestWorkflow_WriteReleaseWorkflow_Good(t *testing.T) {
	t.Run("writes the embedded template to the default path", func(t *testing.T) {
		...
		assert.Contains(t, content, "echo \"short_sha=$short_sha\"")
		...
		assert.Contains(t, content, "echo \"has_root_go_work=$has_root_go_work\"")
		assert.Contains(t, content, "echo \"has_root_wails_json=$has_root_wails_json\"")
		assert.Contains(t, content, "echo \"has_subtree_package_json=$has_subtree_package_json\"")
		assert.Contains(t, content, "echo \"has_subtree_deno_manifest=$has_subtree_deno_manifest\"")
		...
		assert.Contains(t, content, "match = re.match(r\"^\\s*type:\\s*(.+?)\\s*$\", line)")
		...
		assert.Contains(t, content, "steps.discovery.outputs.configured_build_type == 'node'")
		...
		assert.Contains(t, content, "steps.discovery.outputs.has_root_cmakelists == 'true' || steps.discovery.outputs.has_docs_config == 'true'")
		...
		assert.Contains(t, content, "choco install rustup.install -y")
		...
		assert.Contains(t, content, "steps.discovery.outputs.primary_stack_suggestion == 'wails2'")
		assert.Contains(t, content, "runner.os == 'Linux' && (steps.discovery.outputs.primary_stack_suggestion == 'wails2' || steps.discovery.outputs.configured_build_type == 'wails')")
		...
		assert.Contains(t, content, "core build --ci --targets")
		...
		assert.Contains(t, content, "echo \"value=${build_name}\" >> \"${GITHUB_OUTPUT}\"")
