...
func TestBuildCmd_RunReleaseWorkflow_Good(t *testing.T) {
	...
	t.Run("writes to the conventional workflow path by default", func(t *testing.T) {
		...
		assert.Contains(t, content, "npm-build:")
		...
		assert.Contains(t, content, "echo \"npm_requested=$npm_requested\"")
		...
		assert.Contains(t, content, "steps.discovery.outputs.npm_requested == 'true'")
